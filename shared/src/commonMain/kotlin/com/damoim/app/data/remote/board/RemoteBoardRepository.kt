 ...
 *
 * 변경(mutation)은 서버 응답 바디를 대부분 버리고 [RemoteBus.invalidate]로 전 화면 재조회에 의존한다.
 */
class RemoteBoardRepository(private val api: ApiClient) : BoardRepository {
    ...
    override fun observeBoardHome(): Flow<BoardHomeData> =
        reactiveFlow(BoardHomeData(emptyList(), emptyList())) {
    ...
    override fun observePostDetail(postId: Long): Flow<PostDetail?> = reactiveFlow<PostDetail?>(null) {
    ...
    override suspend fun deletePost(postId: Long): DataResult<Unit> =
        api.deleteUnit(ApiRoutes.Board.post(postId)).also { RemoteBus.invalidate() }
    ...
    override suspend fun toggleLike(postId: Long): DataResult<Unit> =
        api.postUnit(ApiRoutes.Board.like(postId)).also { RemoteBus.invalidate() }
    ...
    override suspend fun clearPollVote(postId: Long): DataResult<Unit> =
        api.deleteUnit(ApiRoutes.Board.pollVote(postId)).also { RemoteBus.invalidate() }
    ...
    override suspend fun applyRecruit(postId: Long): DataResult<Boolean> {
        ...
        return when (result) {
            ...
            is DataResult.Failure ->
                // 정원 마감/중복 신청은 실패가 아니라 '신청 불가(false)'로 처리(계약).
    ...
    override suspend fun search(query: String): DataResult<SearchResults> =
        api.getData<SearchResultResponseDto>(ApiRoutes.Board.SEARCH, mapOf("q" to query))
            ...
            .also { RemoteBus.invalidate() } // 최근 검색어 기록 → 추천 화면 갱신

    override fun observeSearchSuggestions(): Flow<SearchSuggestions> =
        reactiveFlow(SearchSuggestions(emptyList(), emptyList())) {
    ...
    override suspend fun clearRecentSearches(): DataResult<Unit> =
        api.deleteUnit(ApiRoutes.Board.SEARCH_RECENT).also { RemoteBus.invalidate() }
