...
object DamoimStrings {
    ...
    const val TOAST_RECRUIT_CANCELED = "모집 신청을 취소했어요"
    ...
    const val RECRUIT_CANCEL_BUTTON = "신청 취소"
    ...
    const val RESOURCE_UPLOAD_HINT = "사진 · 영상 · PDF · 한글(hwp) · 오피스(ppt·xls·doc) 등\n한 파일당 최대 100MB"
    ...
    const val PROFILE_ROW_CLUB_PROFILE = "동아리 프로필"
    const val PROFILE_ROW_NOTIFICATION = "알림 설정"
    const val PROFILE_ROW_SWITCH = "동아리 전환"
    // 44 동아리별 프로필 수정
    const val CLUB_PROFILE_EDIT_TITLE = "동아리 프로필"
    const val CLUB_PROFILE_EDIT_DESC = "이 동아리에서 보일 이름이에요. 동아리마다 다르게 설정할 수 있어요."
    const val CLUB_PROFILE_NAME_LABEL = "표시 이름"
    const val CLUB_PROFILE_NAME_PLACEHOLDER = "이 동아리에서 쓸 이름"
    const val TOAST_CLUB_PROFILE_SAVED = "동아리 프로필을 저장했어요"
    ...
    const val SETTINGS_WITHDRAW_ACCOUNT = "회원 탈퇴"
    const val WITHDRAW_TITLE = "정말 탈퇴할까요?"
    const val WITHDRAW_BODY = "탈퇴하면 계정과 모든 활동 정보가 사라지고 되돌릴 수 없어요. 혼자 남은 동아리는 함께 삭제돼요."
    const val WITHDRAW_CONFIRM = "탈퇴하기"
    const val SETTINGS_DELETE_CLUB = "동아리 삭제"
    fun clubDeleteTitle(name: String) = "'$name'을(를)\n삭제할까요?"
    const val CLUB_DELETE_BODY = "동아리의 모든 게시글·자료·일정이 영구히 사라지고 되돌릴 수 없어요."
    const val CLUB_DELETE_CONFIRM = "삭제하기"
