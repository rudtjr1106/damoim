...
object DamoimStrings {
    ...
    const val LOGIN_KAKAO_START = "카카오로 시작하기"
    ...
    // 32 시작하기
    const val START_SUBTITLE = "동아리에 참여하거나 새로 만들 수 있어요"
    const val START_JOIN_TITLE = "가입 코드로 참여하기"
    const val START_JOIN_SUBTITLE = "동아리장에게 받은 코드가 있어요"
    const val START_CREATE_TITLE = "새 동아리 만들기"
    const val START_CREATE_SUBTITLE = "내가 동아리장이 되어 개설해요"
    fun startGreeting(name: String) = "반가워요, ${name}님!\n어떻게 시작할까요?"

    // 03 가입 코드 입력
    const val JOINCODE_TITLE = "가입 코드를\n입력해주세요"
    const val JOINCODE_SUBTITLE = "동아리장에게 받은 6자리 코드를 입력하면\n가입 신청이 전달돼요"
    const val JOINCODE_TEST_HINT = "테스트 코드 — DAMOIM: 신청 완료 · REJECT: 거절 · EXPIRE: 오류"
